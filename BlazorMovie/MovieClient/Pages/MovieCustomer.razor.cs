...
public partial class MovieCustomer
{
    ...
    private async Task NameSortParm()
    {
        ...
        movies = (await _httpClient.GetFromJsonAsync<List<MovieViewModel>>($"api/Customer/Movie?searchString={searchString}&orderBy={sort}&index={index}"))!;
    }
    ...
    {
        index++;
        movies.AddRange((await _httpClient.GetFromJsonAsync<List<MovieViewModel>>($"api/Customer/Movie?searchString={searchString}&orderBy={sort}&index={index}"))!);
    }
}
