...
namespace MLModel_WebApi
{
    public class Program
    {
        ...
        static async Task PredictHandler(HttpContext http)
        {
            ...
            var predictionEnginePool = http.RequestServices.GetRequiredService<PredictionEnginePool<MLModel.ModelInput, MLModel.ModelOutput>>();

            // Deserialize HTTP request JSON body
            var body = http.Request.Body as Stream;
            var input = await JsonSerializer.DeserializeAsync<MLModel.ModelInput>(body);

            // Predict
            MLModel.ModelOutput prediction = predictionEnginePool.Predict(input);
